package com.pangli.puzzle.widget;
...
public class PuzzleGameLayout extends RelativeLayout implements OnClickListener {
        ...
        for (int i = 0; i < mPuzzleGameItems.length; i++) {
            ...
            if (i % mColumn != 0) {
                lp.addRule(RelativeLayout.RIGHT_OF,//
                        mPuzzleGameItems[i - 1].getId());
            }
            // 如果不是第一行，//设置纵向边距，非最后一行
            if ((i + 1) > mColumn) {
                lp.topMargin = mMargin;
                lp.addRule(RelativeLayout.BELOW, mPuzzleGameItems[i - mColumn].getId());
            }
            addView(item, lp);
        }
    ...
    private void exchangeView() {
        ...
        TranslateAnimation animSecond = new TranslateAnimation(0, mFirst.getLeft() - mSecond.getLeft(), 0, mFirst
